...
def propogation(op2):
    ...
    if(_int_type >= 0): 
        ...
        if(True):
            ...
            srwl_uti_save_intens_ascii(wp_arI, wfr2.mesh,"Intensity_matchMask(20.6m).dat", 0, ['Photon Energy', 'Horizontal Position', 'Vertical Position', ''], _arUnits=['eV', 'm', 'm', sValUnitName])
        return wp_arI           
...
def main():
    ...
    global xRatio
    global yRatio
    xRatio =8e-7 /((srwl_bl.wfr.mesh.xFin - srwl_bl.wfr.mesh.xStart)/2048)  #pixel_size * pixel_num / (srwl_bl.wfr.mesh.xFin - srwl_bl.wfr.mesh.xStart)
    yRatio =8e-7 /((srwl_bl.wfr.mesh.xFin - srwl_bl.wfr.mesh.xStart)/2048) #pixel_size * pixel_num / (srwl_bl.wfr.mesh.yFin - srwl_bl.wfr.mesh.yStart)
